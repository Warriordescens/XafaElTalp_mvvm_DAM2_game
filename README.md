# 🔨 Xafa El Talp | Jetpack Compose Game

## 🚀 Key Features
- **State-Driven UI**: La interfaz reacciona instantáneamente a los cambios en el ViewModel usando `StateFlow`.
- **Custom Animations**: Implementación de `bounceClick` y transiciones suaves entre pantallas.
- **Coroutines Power**: Lógica de juego optimizada para no bloquear el hilo principal.

---

<p align="center">
  <img src="https://img.shields.io/badge/Status-In%20Development-green?style=for-the-badge&logo=android" />
  <img src="https://img.shields.io/badge/Kotlin-1.9+-purple?style=for-the-badge&logo=kotlin" />
  <img src="https://img.shields.io/badge/UI-Jetpack%20Compose-blue?style=for-the-badge&logo=jetpackcompose" />
</p>

---

## 🎮 Sobre el Proyecto
**Xafa El Talp** es un videojuego móvil desarrollado de forma nativa para Android. El proyecto se centra en la implementación de patrones de diseño modernos y una experiencia de usuario fluida mediante animaciones y estados reactivos.

* **Arquitectura:** MVVM (Model-View-ViewModel).
* **Gestión de Estado:** `StateFlow` para una reactividad robusta.
* **Navegación:** Type-safe Navigation con Jetpack Compose.
* **UI/UX:** Diseño personalizado con efectos de escalado (`bounceClick`) y feedback visual dinámico.

---

## 🚀 Características Técnicas
- [x] **Game Loop Asíncrono:** Uso de `Coroutines` para gestionar el cronómetro y el spawn aleatorio de entidades.
- [x] **Navegación Avanzada:** Flujo completo desde Login -> Menú Principal -> Gameplay.
- [x] **UI Adaptable:** Layouts optimizados mediante `LazyVerticalGrid` y modificadores personalizados.
- [x] **Clean Code:** Separación estricta de responsabilidades (UI, Lógica y Datos).

---

## 🛠️ Stack Tecnológico
<p align="left">
  <a href="https://kotlinlang.org/" target="_blank"> <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/kotlin/kotlin-original.svg" alt="kotlin" width="40" height="40"/> </a>
  <a href="https://developer.android.com/studio" target="_blank"> <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/android/android-original.svg" alt="android" width="40" height="40"/> </a>
  <a href="https://developer.android.com/jetpack/compose" target="_blank"> <img src="https://raw.githubusercontent.com/at-shoji/jetpack-compose-logos/main/logos/jetpack_compose_icon_rgb.svg" alt="jetpack-compose" width="50" height="50"/> </a>
</p>

---

## 👷 Autor
**Alex Hernández Guerrero** - *Desarrollador Multiplataforma*
* [LinkedIn](https://www.linkedin.com/in/alex-hern%C3%A1ndez-guerrero-40830731b/)

---
<p align="center">
  <img src="https://capsule-render.vercel.app/render?type=waving&color=F4E3B1&height=100&section=footer" />
</p>
